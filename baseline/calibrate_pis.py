...
def calibrate_pis(pis_guess,HH,i_ini,pop_ini,pir,pid,pis1_shr_target,pis2_shr_target,RplusD_target,phii,C,N,scale1,scale2):
    ...
    pis1 = pis_guess[0]/scale1
    pis2 = pis_guess[1]/scale2
    pis3 = pis_guess[2]
    ...
    for j in np.arange(HH):
        T[j]   = pis1*S[j]*(C**2)*I[j] + pis2*S[j]*(N**2)*I[j] + pis3*S[j]*I[j]
        S[j+1] = S[j]-T[j]
        I[j+1] = I[j]+T[j]-(pir+pid)*I[j]
        R[j+1] = R[j]+pir*I[j]
        D[j+1] = D[j]+pid*I[j]
...
def calibrate_pis_zeros(pis_guess,HH,i_ini,pop_ini,pir,pid,pis1_shr_target,pis2_shr_target,RplusD_target,phii,C,N,scale1,scale2):
